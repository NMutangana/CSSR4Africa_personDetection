/*
    ...
    This driver reads a sequence lines from an input file personDetectionRGBInput.txt.
...
int main(int argc, char **argv) { 
    ...
    ros::Publisher image_pub = nh.advertise<sensor_msgs::Image>("/naoqi_driver/camera/front/image_raw", 1000);

    // const char inputWindowName[MAX_STRING_LENGTH] = "Input Image";
    cv::Mat inputImage;
    // cv::namedWindow(inputWindowName, cv::WINDOW_AUTOSIZE);

    const char input_filename[MAX_FILENAME_LENGTH] = "personDetectionRGBInput.txt";    
    ...
    if ((fp_in = fopen(input_path_and_filename,"r")) == 0) {
        printf("Error can't open input personDetectionRGBInput.txt\n");
        prompt_and_exit(1);
    }

    printf("Opened input file: personDetectionRGBInput.txt\n\n");
        ...
        if (end_of_file != EOF) {          
            ...
            if (strcmp(filename, "camera") != 0) {
                ...
                if (!inputImage.empty()) {
                    ...
                    img_bridge.encoding = "bgr8"; // Assuming it's a color image
                ...
                if (camera.isOpened()) {
                    ...
                    do {
                        ...
                        cv_bridge::CvImage img_bridge;
                        img_bridge.image = inputImage;
                        img_bridge.encoding = "bgr8"; // Since it's a color image
                        sensor_msgs::ImagePtr ros_img_msg = img_bridge.toImageMsg();
